 ...
 * <p>
 * Pushed application bundle is merely a collection of files inside a directory. All files in this
 * directory is considered to belong to the bundle and directory should not have any other files or
 * nested directories.
 *
...
public class YarnPushApplication extends AbstractClientApplication<String, YarnPushApplication> {
  ...
  /**
   * Associates a new {@link Properties} with a name. These properties will be serialised into a
   * common properties format with a given config file name.
   *
  ...
  /**
   * Run a {@link SpringApplication} build by a {@link SpringApplicationBuilder} using an empty
   * args.
   *
   * @see #run(String...)
   */
  ...
  public String run(String... args) {
    if (!StringUtils.hasText(applicationVersion)) {
      throw new SpringApplicationException("Error executing a spring application",
          new IllegalArgumentException("Instance id must be set"));
    }
    ...
    if (StringUtils.hasText(applicationBaseDir)) {
      appProperties.setProperty("spring.yarn.applicationDir",
          applicationBaseDir + applicationVersion + "/");
    }
    ...
    return template.execute(new SpringApplicationCallback<String>() {
      ...
      public String runWithSpringApplication(ApplicationContext context) throws Exception {
        ...
        if (client instanceof ApplicationYarnClient) {
          ((ApplicationYarnClient) client)
              .installApplication(new ApplicationDescriptor(applicationdir));
        } else {
